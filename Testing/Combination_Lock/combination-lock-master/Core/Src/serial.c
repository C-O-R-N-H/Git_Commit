...
void read_velocity(int8_t *horizontal_vel, int8_t *vertical_vel, SerialPort *serial_port)
{
	// Read a data packet from serial that has the velocity
	uint8_t data_packet_input_buffer[HEADER_SIZE] = {0};
	uint16_t data_packet_size = SerialInputPacketHeader(data_packet_input_buffer, serial_port);

	// Copy the data to a header structure
	Header incoming_header = {0};
	memcpy(&incoming_header, data_packet_input_buffer, sizeof(Header));

	if (incoming_header.message_type == VELOCITY)
	{
		Velocity vel_data = {0};
		uint8_t success = SerialInputDataPacket(&vel_data, sizeof(vel_data), serial_port);

		if (success > 0)
		{
			*horizontal_vel = vel_data.horizontal_vel;
			*vertical_vel = vel_data.vertical_vel;
		}
	}
}
