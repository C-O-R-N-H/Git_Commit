...
void check_condition()
{
	if (GPIOA->IDR & 0x01)  // Button is pressed
	{
		...
		// Last_period is PWM reading, lidar_distance is I2C reading
		if (abs(last_period - target_distance) <= LIDAR_THRESHOLD || abs(lidar_distance - target_distance) <= LIDAR_THRESHOLD)
		{
